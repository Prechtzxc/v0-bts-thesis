...
export default function QRVerificationPage() {
  ...
  const handleMarkAsClaimed = async () => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to mark as claimed",
      })
    }
  }
  ...
  const handleManualVerify = async () => {
    ...
    if (student) {
      ...
      if (studentApplication) {
        // MUST await async eligibility check
        const eligibilityResult = await markStudentAsEligible(profile?.studentId || student.id)
        
        // Check if already claimed
        const alreadyClaimed = await hasStudentClaimed(student.id)
        ...
        toast({
          ...
          description: studentApplication 
            ? `Student found but application status is: ${studentApplication.status}`
            : "Student found but has no approved application",
        })
      }
    } else {
      setVerificationResult({
  ...
  const handleQRCodeResult = async (result: string) => {
    try {
      ...
      console.log("[v0] QR Code scanned raw result:", result)

      // Check for new simple format: BTS:studentId
      if (result.startsWith("BTS:")) {
        studentIdFromQR = result.replace("BTS:", "")
        console.log("[v0] Simple QR format detected, student ID:", studentIdFromQR)
      }
      // Check if QR code is encrypted (legacy)
        ...
        console.log("[v0] Decrypted QR data, student ID:", studentIdFromQR)
      } else {
        // Handle legacy JSON format
        try {
          data = JSON.parse(result)
          studentIdFromQR = data.studentId || data.id
          console.log("[v0] JSON QR format, student ID:", studentIdFromQR)
        } catch {
          // If not JSON, treat the whole string as student ID
          studentIdFromQR = result
          console.log("[v0] Plain text QR, using as student ID:", studentIdFromQR)
        }
      }
      ...
      console.log("[v0] Found student:", student ? student.id : "Not found")
      ...
      if (student) {
        ...
        if (profile) {
          // Check if student has an approved application
          const applications = getApplications()
          console.log("[v0] Total applications:", applications.length)

          const studentApplication = applications.find((app) => app.studentId === student.id && app.status === "approved")
          console.log("[v0] Found approved application:", studentApplication ? "Yes" : "No")

if (studentApplication) {
  // MUST await async eligibility check
  const eligibilityResult = await markStudentAsEligible(profile.studentId || student.id)
  console.log("[v0] Eligibility result:", eligibilityResult)
  
  // Check if already claimed - MUST await async call
  const alreadyClaimed = await hasStudentClaimed(student.id)
  setIsClaimed(alreadyClaimed)
  ...
  profilePicture: profile.profilePicture || student.profilePicture,
            ...
            toast({
              variant: "destructive",
              title: "Verification failed",
              description: "Student found but application not approved",
            })
          }
        }
      } else {
  ...
  <Button className="w-full" onClick={handleMarkAsClaimed}>
    Mark as Claimed
                ...
                <div className="rounded-lg bg-red-50 p-4 text-red-700 border border-red-200">
                  ...
                  <p className="mt-1 text-sm">This student is not eligible or not found in the system.</p>
                </div>
              )
            ) : (

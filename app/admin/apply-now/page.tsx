...
export default function ApplyNowPage() {
  ...
  useEffect(() => {
    const loadApplications = async () => {
      try {
        const newApplications = await getNewScholarApplications()
  ...
  const handleApproveApplication = async (application: NewScholarApplication) => {
    try {
      ...
      const updatedApp = await updateNewScholarApplication(application.id, { status: "approved" })
      if (updatedApp) {
  ...
  const handleRejectApplication = async (applicationId: string) => {
    try {
      const updatedApp = await updateNewScholarApplication(applicationId, { status: "rejected" })
      if (updatedApp) {

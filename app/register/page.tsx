...
import { createUser, createApplication, initializeStorage, checkEmailRegistrationStatus } from "@/lib/storage"
...
export default function RegisterPage() {
  ...
  const validateEmail = async () => {
    ...
    // Check email registration status with detailed error handling
    const emailStatus = await checkEmailRegistrationStatus(formData.email)
    
    if (!emailStatus.isPreApproved) {
      setErrors({ email: emailStatus.errorMessage || "This email cannot be used for registration" })
      toast({
        variant: "destructive",
        title: emailStatus.isRegistered ? "Email already registered" : "Email not authorized",
        description: emailStatus.errorMessage || "Please contact the administrator.",
      })
      return false
    }
  ...
  const validateCurrentStep = async () => {
    ...
    if (step === 1) {
      const requiredFields: (keyof FormData)[] = ["fullName", "address", "contactNumber", "age", "barangay"]
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // MUST await createUser before using newUser.id - async operation to Supabase
      ...
      // MUST await application creation to ensure database persistence

...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (user?.profileData) {
      ...
      const fullName = typeof profile === 'object' && 'fullName' in profile 
        ? profile.fullName 
        : user.name || 'Unknown'
      ...
      setPersonalInfo({
        ...
        email: user.email || '',
        contactNumber: (typeof profile === 'object' && 'contactNumber' in profile ? profile.contactNumber : '') || '',
        address: (typeof profile === 'object' && 'address' in profile ? profile.address : '') || '',
        age: (typeof profile === 'object' && 'age' in profile ? profile.age : '') || '',
        barangay: (typeof profile === 'object' && 'barangay' in profile ? profile.barangay : '') || '',
        isPWD: (typeof profile === 'object' && 'isPWD' in profile ? profile.isPWD : false) || false,
      })

      if (typeof profile === 'object' && 'course' in profile) {
        setEducationInfo({
          school: (typeof profile === 'object' && 'schoolName' in profile ? profile.schoolName : '') || '',
          course: (typeof profile === 'object' && 'course' in profile ? profile.course : '') || '',
          yearLevel: (typeof profile === 'object' && 'yearLevel' in profile ? profile.yearLevel : '') || '',
          studentId: (typeof profile === 'object' && 'studentId' in profile ? profile.studentId : '') || '',
        })
      }
  ...
  const handlePersonalUpdate = async () => {
    ...
    try {
      ...
      // MUST await async updateUser function
  ...
  const handleSecurityUpdate = async () => {
    ...
    setIsLoading(true)
    try {
      // MUST await async updateUserPassword function
      const success = await updateUserPassword(user.id, securityInfo.currentPassword, securityInfo.newPassword)
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="20"
                    type="number" // Added type="number" for age input
                  />
                </div>
                <div className="space-y-2">

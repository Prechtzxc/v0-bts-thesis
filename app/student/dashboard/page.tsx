      ...
      const loadDashboardData = async () => {
        try {
          ...
          if (claimed) {
            ...
            if (record) {
              setClaimedDate(record.claimedDate || record.claimedAt)
            }
          }
          
          // Get student application data from Supabase
          const applications = await getApplicationsByStudentId(user.id)
          const documents = await getDocumentsByStudentId(user.id)
          const history = await getApplicationHistoryByStudentId(user.id)

...
export function AdminLayout({ children }: AdminLayoutProps) {
  ...
  // Check auth only once after loading completes - prevent chained redirects
  useEffect(() => {
  ...
  useEffect(() => {
    if (user && user.role === "admin") {
      const fetchNotifications = async () => {
        try {
          const adminNotifications = await getNotificationsByUserId(user.id)
  ...
  const navigationItems = allNavigationItems.filter((item) => hasPermission(user?.admin_role as AdminRole, item.permission))

  const adminRoleLabel = user?.admin_role ? getAdminRoleLabel(user.admin_role as AdminRole) : "Administrator"

  const unreadCount = notifications.filter((n) => !(n as any).is_read).length
